...
import RequestProductModal from "./requestProductModal.component";
...
export default function ProductRequestBanner() {
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-4 px-6 py-4 bg-white border shadow-sm shadow-xl border-slate-200 rounded-xl md:rounded-full sm:flex-row sm:py-5 sm:gap-6">
      {/* Left section */}
      {open && <RequestProductModal open={open} onClose={() => setOpen(false)} />}
      ...
      <button
        ...
        className="text-[#B42318] border border-[#F04438] rounded-full px-4 py-2 text-sm font-semibold bg-[#ffecec] transition-colors duration-200"
      >
        Post Request
      </button>
    </div>
